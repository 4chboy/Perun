...
namespace perun
{
    ...
    {
        window = Window::Create({.width = 1280, .height = 720, .title = "PerunApp", .fullScreen = false, .vSync = false});
        context = GraphicsContext::Create(window, GraphicsAPIType::OpenGL);
        api = GraphicsAPI::Create(context);

        window->GetEventSystem().AddEventListener({.eventType = EventType::WindowResized, .dispatchFn = [&](Event& event) -> bool {
