...
namespace perun
{
    bool macos_input::is_key_down(key_code key) const noexcept
    {
        return false;
    }

    bool macos_input::is_mouse_button_down(mouse_code button) const noexcept
    {
        return false;
    }

    std::pair<float, float> macos_input::get_mouse_pos() const noexcept
    {
        return {.0F, .0F};
    }
