...
namespace perun
{
    ...
    [[nodiscard]] uint32_t OpenGLAPI::GenerateTexture(uint32_t width, uint32_t height, const uint8_t* data) const
    {
        ...
        glContext.TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, reinterpret_cast<const void*>(data));
    ...
    uint32_t OpenGLAPI::GenerateShader(const ShaderInfo& shaderInfo) const
    {
        ...
        auto shaderSource = std::string{std::istreambuf_iterator<char>{shaderFile}, std::istreambuf_iterator<char>{}};
