...
namespace perun
{
    ...
    public:
        ...
        [[nodiscard]] inline uint8_t* GetData() noexcept
        {
            return data;
        }
        [[nodiscard]] inline const uint8_t* GetData() const noexcept
        {
            return data;
        }

        [[nodiscard]] inline uint8_t& operator[](size_t index) noexcept
        {
            return data[index];
        }
        ...
        [[nodiscard]] static std::unique_ptr<Texture> Create(std::unique_ptr<GraphicsAPI>& api, uint32_t textureWidth, uint32_t textureHeight);
        [[nodiscard]] static std::unique_ptr<Texture> Load(std::unique_ptr<GraphicsAPI>& api, const std::string& filePath);
