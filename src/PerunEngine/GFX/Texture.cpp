...
namespace perun
{
    ...
    std::unique_ptr<Texture> Texture::Create(std::unique_ptr<GraphicsAPI>& api, uint32_t textureWidth, uint32_t textureHeight)
    {
        return std::unique_ptr<Texture>{new Texture{api, textureWidth, textureHeight}};
    }

    std::unique_ptr<Texture> Texture::Load(std::unique_ptr<GraphicsAPI>& api, const std::string& filePath)
    {
